...
echo "1. 🔴 RED: Ensure required features exist and start with failing tests"
for feature in "${FEATURES[@]}"; do
    echo "   - Verify @/features/${feature} is implemented with proper types"
done
echo "   - Run: yarn workspace frontend test src/widgets/${WIDGET_NAME}/ (should pass with mocks)"
echo "2. 🟢 GREEN: Integrate real feature components"
echo "   - Replace TODO comments with actual feature integrations"
echo "   - Update widget tests to use real feature data"
echo "   - Ensure proper error handling and loading states"
echo "3. 🔵 BLUE: Refactor layout and optimize performance"
echo "4. Use MCP tools for comprehensive test coverage:"
echo "   - Create decision tables for widget layout scenarios"
echo "   - Test different feature combinations and states"
echo "   - Generate edge case tests: mcp__testing-mcp__generate_tests"
echo "   - Test responsive behavior and accessibility"
echo "5. Use the widget in a page:"
echo "   import { ${PASCAL_CASE_NAME}Widget } from '@/widgets/${WIDGET_NAME}';"
echo "6. Choose between standard or tabbed layout based on UX requirements"