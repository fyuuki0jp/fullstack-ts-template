...
echo "1. 🔴 RED: Start with failing tests and requirements"
echo "   - Update test cases in ${ADAPTER_NAME}.spec.ts with actual scenarios"
echo "   - Define configuration requirements and error cases"
echo "   - Run: yarn workspace backend test src/shared/adapters/ (should fail initially)"
echo "2. 🟢 GREEN: Implement minimal adapter functionality"
echo "   - Implement the adapter logic in ${ADAPTER_NAME}.ts"
echo "   - Add proper error handling and Railway Result patterns"
echo "   - Make tests pass with minimal viable implementation"
echo "3. 🔵 BLUE: Refactor for production readiness"
echo "4. Use MCP tools for comprehensive test coverage:"
echo "   - Create decision tables for adapter configuration scenarios"
echo "   - Generate edge case tests: mcp__testing-mcp__generate_tests"
echo "   - Test error handling, retries, and timeout scenarios"
echo "   - Analyze coverage: mcp__testing-mcp__analyze_coverage"
if [ "$ADAPTER_TYPE" = "db" ]; then
    echo "5. Register the adapter in your server configuration"
elif [ "$ADAPTER_TYPE" = "external" ]; then
    echo "5. Use the service in your features via dependency injection"
fi
echo "6. Add monitoring and logging for production use"